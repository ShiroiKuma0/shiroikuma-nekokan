...
/**
 * shiroikuma-nekokan fork — who is allowed through the data door ([SkAutomationProvider]), and how
 * that is decided. Copied from 自由作業盤's `AutomationCallers`, which is deliberately
 * self-contained and app-independent; only the package line differs.
 *
 * ## Why not a token
 *
 ...
 * turn: strictly weaker than the token it replaces. For *this* app that data is loyalty- and
 * membership-card barcodes — scannable credentials — so the distinction is not academic.
 *
...
object SkAutomationCallers {
     ...
     *
     * Both hashes were re-derived from the signed release APKs in `~/tmp` rather than copied on
     * trust (shiroikuma-nekokan, 2026-09-04) — they matched the contract's values exactly.
     */
     ...
     *
     * `signingInfo` and `GET_SIGNING_CERTIFICATES` are API 28, and **this app's minSdk is 23** —
     * lower than the reference's, so the legacy branch matters here more, not less. On a 23–27
     * device the flag is accepted and `signingInfo` comes back null, so WITHOUT this branch the
     * door would refuse every caller: a total failure that never appears on 白い熊's phone (which
     * reports API 31) and would only surface on an older one. The deprecated array is the correct
     * answer there, not a compromise — before key rotation existed, `signatures` WAS the signing
     * certificate.
     */
