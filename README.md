...
**📥 Latest release: [`2.45.0+001`](https://github.com/ShiroiKuma0/shiroikuma-nekokan/releases/latest)** — [all releases & APK downloads »](https://github.com/ShiroiKuma0/shiroikuma-nekokan/releases)
...
The same backup, headlessly. A token-gated broadcast makes the app export itself with no window ever opening — one ZIP, written either to its own directory or to an absolute path the caller names — and reply with the real path and byte size, so a task on the phone can back this app up unattended alongside its sister apps. Progress comes back as **real counts, never a percentage** (`項目 123/456 — All cards`). The caller is told which categories exist **and which ones this app says should start ticked**, so a backup picker never has to guess. A running export can be **stopped from outside**: the cancel unwinds it at the next entry boundary, deletes the partial file so the backup folder is left exactly as it was found, and reports `cancelled` — and it is harmless to send when nothing is running. The switch is **off** until you turn it on, in the Export/Import section right below the export rows; the tap-to-copy token is what any caller must present, and regenerating it locks out every pasted copy.
